...
/**
 * The header chip is the first entry of the session header actions: its order sits below
 * every shipped occupant (the subagent catalog at `-30`, the agent-preset selector at
 * `-10`, the header actions at `0` and up), so the indicator is drawn left of the
 * selected mode rather than behind it.
 */
const HEADER_ENTRY = { name: 'conversation.session.header.actions', id: 'deepseek-peaks', order: -40 }
